...
export default class VideoControls extends React.Component {
	...
	componentWillUnmount() {
		...
		document.body.removeEventListener('mouseup', (evt) =>
	...
	getMaxRightBound() {
		return (
			100 -
			(this.slider.current.clientWidth / this.bar.current.clientWidth) * 100
		);
	}
	...
	render() {
		return (
			<section className="video-controls">
				<div
					ref={this.bar}
					onMouseDown={(evt) => this.handleSeek(evt)}
					className="progress-bar">
					<div style={{ position: 'relative' }}>
						<div
							style={{ position: 'absolute', left: `${this.state.sliderPos}%` }}
							onMouseDown={(evt) => this.handleDragging(evt)}>
